...
class SearchViewModelTest {
    ...
    @Test
    fun `when query is blank, state must return Idle`() = runTest {
    ...
    fun `searchMovies emits Loading then Success`() = runTest {
        ...
        coEvery { moviesRepository.search("batman") } returns flow {
    ...
    @Test
    fun `searchMovies emits Loading then Error`() = runTest {
        ...
        coEvery { moviesRepository.search("fail") } returns flow {
            emit(ResponseState.Loading)
            emit(errorState)
        }
        val collected = mutableListOf<ResponseState<List<MovieModel>>>()
    ...
    @Test
    fun `searchMovies ignores Idle emissions from repository`() = runTest {
