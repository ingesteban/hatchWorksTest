...
class MoviesPagingSourceTest {
    ...
    @Test
    fun `load returns Page on success first page`() = runTest(testDispatcher) {
        ...
        pagingSource = MoviesPagingSource { page ->
            assertEquals(1, page)
            response
        }
    ...
    @Test
    fun `load returns Page on success middle page`() = runTest(testDispatcher) {
    ...
    @Test
    fun `load returns Page with nextKey null when last page`() = runTest(testDispatcher) {
        val expectedMovies = listOf<NetworkMovieResponse>(mockk())
        val response = NetworkMoviesResponse(
            page = 5,
            results = expectedMovies,
    ...
    @Test
    fun `load returns Error on exception`() = runTest(testDispatcher) {
        val exception = IOException("Network failure")

        pagingSource = MoviesPagingSource { _ ->
            throw exception
        }
    ...
    fun `getRefreshKey returns null when no anchorPosition`() {
        val state = PagingState<Int, NetworkMovieResponse>(
            pages = emptyList(),
            anchorPosition = null,
            config = PagingConfig(pageSize = 20),
            leadingPlaceholderCount = 0
