...
class MoviesRepositoryImpl
    ...
    constructor(
        ...
        override fun nowPlaying(): Flow<ResponseState<List<MovieModel>>> = fetchMovies { moviesDataSource.nowPlaying() }

        override fun trending(): Flow<ResponseState<List<MovieModel>>> = fetchMovies { moviesDataSource.trending() }

        override fun upcoming(): Flow<ResponseState<List<MovieModel>>> = fetchMovies { moviesDataSource.upcoming() }

        override fun popular(): Flow<ResponseState<List<MovieModel>>> = fetchMovies { moviesDataSource.popular() }
        ...
        private fun fetchMovies(serviceCall: suspend () -> NetworkMoviesResponse): Flow<ResponseState<List<MovieModel>>> =
            flow { emit(serviceCall()) }
                .mapper(moviesMapper)
                .onError(NetworkErrorMapper())
                .flowOn(ioDispatcher)
